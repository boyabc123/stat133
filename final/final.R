...
name <- "HungWei Lin"
github.name <- "boyabc123"
email.address <- "boyabc123@berkeley.edu"
...
by(iris$Petal.Length, iris$Species, boxplot)
...
color = rainbow(3,alpha = 0.25)
plot(iris$Petal.Length,iris$Petal.Width,
     xlab = "Petal Length", ylab= "Petal Width", col = color[iris$Species] )
...
plot( 1:150, iris$Sepal.Length / iris$Petal.Length, col=color[iris$Species])
...
plot(infants$gestation , infants$bwt, pch="*",xlab="gestation",ylab="birthweight",col='red')
abline(v=259)
...
plot(density(infants$age,na.rm=T),col='blue',ylim=c(0,0.1),xlab="Mother's age", main="density of mother's age")
hist(infants$age,freq=F,add=T,ylim=c(0,0.1),xlab="Mother's age")
...
max.petal.width <-  sapply(levels(iris$Species), function(s)
  max(iris$Petal.Width[iris$Species==s]))
...
firstColToNames <- function( input ){
  if(is.matrix(input)){
...
for (i in 1:4){
  temp = dice_sum(2,Bvec[i])
  ave.diceRoll[i] = mean( temp )
  sd.diceRoll[i] = sd( temp )
  hist(temp, xlab="sum of dice roll", main = paste("Histogram for B=", Bvec[i]))
}
...
minchin.split <- strsplit(tolower(minchin),split = " ")[[1]]
