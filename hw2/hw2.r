...
plot( OW_weight, family$weight, xlim=c(120,200),ylim=c(120,200) )
...
BerkArea <- housing[housing$city %in% local.cities, colnames(housing) %in% some.housing.variables]
...
BerkArea <- BerkArea[BerkArea$bsqft < unname(quantile(BerkArea$bsqft, probs = seq(0,1,0.99),na.rm=TRUE)[2]) & BerkArea$price < unname(quantile(BerkArea$price, probs = seq(0,1,0.99))[2]),]
...
br5 <- replace(BerkArea$br,which(BerkArea$br > 5),5)
...
plot(BerkArea$year,BerkArea$pricePsqft,
     main = "Housing Prices Per Square Foot vs Year in the Berkeley Area",
     xlab = "Year",
     ylab = "Price per square foot",
     col = brCols, pch = 19, cex = 0.5)
legend(legend = 1:5, fill = rCols, "topright")
