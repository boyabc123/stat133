...
wr.name <- as.character(wr1500m[which((wr1500m$times)==min(wr1500m$times)), which(names(wr1500m)=="athlete")])
...
wr1500m <- as.data.frame(cbind(wr1500m,times_sec))
...
wr1500m <- as.data.frame(cbind(wr1500m,new_year))
...
plot(wr1500m$new_year, wr1500m$times_sec, xlim=c(1892,2014+9/12))
lines(x=c(wr_1998,2014.75),y=c(min(wr1500m$times_sec),min(wr1500m$times_sec)))
...
wr_1944 <- wr1500m$new_year[which(wr1500m$year == 1944)]
abline(v=wr_1944)
abline(v=wr_1998)
text( x= wr_1944,
      y= wr1500m$times_sec[which(wr1500m$year == 1944)]
      ,wr1500m$athlete[which(wr1500m$year == 1944)],cex=0.5,col='blue')
text( x= wr_1998,
      y= wr1500m$times_sec[which(wr1500m$year == 1998)]
      ,wr1500m$athlete[which(wr1500m$year == 1998)],cex=0.5,col='red')
...
plot(x=wr1500m$new_year, 
     y=wr1500m$times_sec, 
     xlim=c(1892,2014+9/12),
     xlab='Year',
     ylab='Time in sec',
     main='1500M World Record vs Year')
lines(x=c(wr_1998,2014.75),y=c(min(wr1500m$times_sec),min(wr1500m$times_sec)))
abline(v=wr_1944)
abline(v=wr_1998)
text( x= wr_1944,
      y= wr1500m$times_sec[which(wr1500m$year == 1944)]
      ,wr1500m$athlete[which(wr1500m$year == 1944)],cex=1,col='blue')
text( x= wr_1998,
      y= wr1500m$times_sec[which(wr1500m$year == 1998)]
      ,wr1500m$athlete[which(wr1500m$year == 1998)],cex=1,col='red')
...
SO2012Ctry <- as.data.frame(cbind(SO2012Ctry,GDP_per_person))
symbols(x= log(SO2012Ctry$pop),
        y= log(GDP_per_person), circles=sqrt(SO2012Ctry$Total))
...
SO2012Ctry_modified = SO2012Ctry[SO2012Ctry$Total>0,]
SO2012Ctry_modified2 = SO2012Ctry[SO2012Ctry$Total == 0,]

plot (x = log(SO2012Ctry_modified2$pop),
      y = log(SO2012Ctry_modified2$GDP_per_person),
      pch=".")

symbols( x = log(SO2012Ctry_modified$pop),
         y = log(SO2012Ctry_modified$GDP_per_person),
         circles=sqrt(SO2012Ctry_modified$Total), add=TRUE )
...
top5 <- order(SO2012Ctry$Total,decreasing = TRUE )

plot (x = log(SO2012Ctry_modified2$pop),
      y = log(SO2012Ctry_modified2$GDP_per_person),
      pch="." , xlab = 'Log of Population', ylab = 'log of GDP per Person', 
      main = 'Log of GDP per Person vs Log of Population',xlim=c(9,22))

symbols( x = log(SO2012Ctry_modified$pop),
         y = log(SO2012Ctry_modified$GDP_per_person),
         circles=sqrt(SO2012Ctry_modified$Total), add=TRUE )

with(SO2012Ctry[top5,][1:5,],
     text(log(pop), log(GDP_per_person), labels=Country, col=c(rainbow(5)) ))
...
symbols( x = wonMedal$longitude,
         y = wonMedal$latitude,
         circles=sqrt(wonMedal$Total), add=TRUE,inches=FALSE )
...
symbols( x = wonMedal$longitude,
                 y = wonMedal$latitude,
                 circles=sqrt(wonMedal$Total), add=TRUE,inches=FALSE,bg=myGold )
...
n.f.athletes = sum(athletes$Sex == 'F')

# What proportion of the participants were women?
frac.women <- sum(athletes$Sex =='F')/nrow(athletes)
...
barplot(table(athletes$Sex,athletes$Sport)[,orderSport],
        beside=TRUE,
        las=2, cex.names=0.5, main='Males and Females in Each Sport')
