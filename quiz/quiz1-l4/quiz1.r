...
height.diffs <- sapply(family$height, function(x) (x-mean(family$height))^2)
...
ppl.old <- family[family$age > mean(family$age),]
ppl.young <- family[family$age <= mean(family$age),]
...
ppl.old.weight <- ppl.old[,colnames(family) %in% c("firstName","weight")]
ppl.young.weight <- ppl.young[,colnames(family) %in% c("firstName","weight")]
...
plot(x = family$age,y = family$bmi, main="BMI vs Age", xlab="Age (Year)", ylab= "BMI")
